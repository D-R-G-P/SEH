...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['dniInfo']) && isset($_POST['pedidoInfo'])) {
        // Obtener los datos del formulario
        $dni = $_POST['dniInfo'];
        $pedido = $_POST['pedidoInfo'];
        ...
        if ($statement->execute()) {
            ...
            $_SESSION['success_message'] = '<div class="notisContent"><div class="notis" id="notis" style="text-align: center;">Pedido realizado correctamente. Si es cambio de permisos, deberá subir los documentos nuevamente.</div></div><script>setTimeout(() => {notis.classList.toggle("active");out();}, 1);function out() {setTimeout(() => {notis.classList.toggle("active");}, 25000);}</script>';
