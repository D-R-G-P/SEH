    ...
    if (isset($_POST['dni'], $_POST['servicio'], $_POST['email'], $_POST['phone'], $_POST['permisos'])) {
        ...
        try {
            ...
            if ($stmt->execute($params)) {
                ...
                $_SESSION['success_message'] = '<div class="notisContent"><div class="notis" id="notis" style="text-align: center;">Usuario solicitado, verifique su bandeja</div></div><script>setTimeout(() => {notis.classList.toggle("active");out();}, 1);function out() {setTimeout(() => {notis.classList.toggle("active");}, 2500);} window.addEventListener("DOMContentLoaded", () => { loadInfo("' . $dni . '", "' . $servicio . '"); });</script>';
                header("Location: " . $_SERVER['HTTP_REFERER']);
                exit();
            } else {
