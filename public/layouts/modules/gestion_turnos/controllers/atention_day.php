...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    try {
        ...
        for ($id = 1; $id <= 7; $id++) {
            ...
            $start_time = $_POST['start_time'][$id] ?? null;
            $end_time = $_POST['end_time'][$id] ?? null;

            $stmt = $pdo->prepare("UPDATE atention_days_turnos SET enabled = ?, start_time = ?, end_time = ? WHERE id = ?");
            $stmt->execute([$enabled, $start_time, $end_time, $id]);
        }
